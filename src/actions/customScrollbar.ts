...
export function customScrollbar(page: HTMLElement): { destroy(): void } {
  ...
  // Коалесцируем обновления в один кадр: MutationObserver/ResizeObserver могут
  // срабатывать сотни раз за рендер страницы, а updateScrollbars делает
  // синхронные reflow-замеры (scrollHeight/getComputedStyle).
  let updateRaf = 0;
  function scheduleScrollbarsUpdate() {
    if (updateRaf) return;
  ...
  scrollEl.addEventListener('scroll', scheduleScrollbarsUpdate);
  ...
  const ro = new ResizeObserver(scheduleScrollbarsUpdate);
  ro.observe(scrollEl);

  const mo = new MutationObserver(scheduleScrollbarsUpdate);
  ...
  return {
    destroy() {
      ...
      scrollEl.removeEventListener('scroll', scheduleScrollbarsUpdate);
