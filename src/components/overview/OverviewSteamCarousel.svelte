<script lang="ts">
  ...
  async function loadSlideMedia(
    ...
    const task = (async () => {
      ...
      if (!customBg && releaseId) {
        ...
        try {
          const release = await fetchReleaseInfoCached(releaseId);
  ...
  // Полные данные слайда (info + скриншоты) тянем не для всех баннеров сразу —
  // это десятки тяжёлых запросов, которые душат загрузку видимого контента.
  const PRELOAD_FULL_LIMIT = 8;
  ...
  async function preloadAllCovers(
    ...
    const fullItems = bannerItems.slice(0, PRELOAD_FULL_LIMIT);
    let cursor = 0;
    const workers = Array.from(
      { length: Math.min(2, fullItems.length) },
      async () => {
        while (cursor < fullItems.length) {
          const banner = fullItems[cursor++];
          if (!banner) continue;
  ...
  function startProgressTimer() {
    ...
    // Обновляем Svelte-состояние не чаще ~7 раз/сек: писать state каждый кадр —
    // это лишний re-render всей карусели 60 раз в секунду. Плавность полосы
    // добивает CSS-transition на .steam-hero__dot-fill.
    let lastStateSync = 0;
