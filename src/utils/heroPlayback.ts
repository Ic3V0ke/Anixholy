...
// Один info-запрос на релиз: и фон, и скриншоты карусели берутся из одного ответа.
const releaseInfoCache = new Map<number, Promise<Record<string, unknown> | undefined>>();

export function fetchReleaseInfoCached(releaseId: number): Promise<Record<string, unknown> | undefined> {
  const cached = releaseInfoCache.get(releaseId);
  if (cached) return cached;

  const api = window.anixApi?.release;
  if (!api?.info) return Promise.resolve(undefined);

  const task = api.info(releaseId, true)
    .then((data) => data?.release as Record<string, unknown> | undefined)
    .catch(() => {
      releaseInfoCache.delete(releaseId);
      return undefined;
    });
  releaseInfoCache.set(releaseId, task);
  return task;
}
