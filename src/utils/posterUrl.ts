...
/**
 * URL для fetch() CDN-ресурсов (JSON, Lottie): в Electron через anix-cdn://,
 * т.к. прямой fetch упирается в CORS. Объём таких запросов мал.
 */
function toCdnFetchUrl(url: string): string {
  const trimmed = unwrapCdnUrl(url?.trim() ?? '');
  if (!trimmed) return '';
  if (!isAnixartCdnUrl(trimmed)) return toCdnProxyUrl(trimmed);
  if (typeof window !== 'undefined' && window.electron) {
...
export async function fetchCdnJson(url: string): Promise<unknown | null> {
  const proxy = toCdnFetchUrl(url.trim());
