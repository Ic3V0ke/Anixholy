<script lang="ts">
  ...
  let {
    ...
    boost, nightMode, autoNext, sleepAfterEp, hasNextEp,
    ontogglePlay, ontoggleMute, onvolumechange, ontoggleUpscale,
    ontoggleBoost, ontoggleNight, ontoggleAutoNext, ontoggleSleep, onnextEpisode,
    onskipOpening,
  ...
  const volumeMax = $derived(boost ? 200 : 100);
...
<div class="watch-page__btn-row">
    ...
    <div class="watch-page__vol-wrap" onclick={(e) => e.stopPropagation()}>
      ...
      <div class="watch-page__vol-slider-wrap">
        <input
          ...
          class:watch-page__vol-slider--boosted={volume > 100}
          min="0"
          max={volumeMax}
          value={sliderValue}
        ...
        />
        {#if volume > 100}
          <span class="watch-page__vol-boost-badge">{volume}%</span>
        {/if}
      </div>
    </div>
  ...
  <div class="watch-page__btns-right">
    ...
    <!-- Следующая серия -->
    {#if useVideo && hasNextEp}
      <button
        type="button"
        class="watch-page__ctrl-btn watch-page__ctrl-btn--text watch-page__next-btn"
      ...
      >
        ...
        <span class="watch-page__next-btn-label">Серия {currentEp + 1}</span>
      </button>
    {/if}
