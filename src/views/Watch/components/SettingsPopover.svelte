<script lang="ts">
  ...
  let {
    ...
    boost, nightMode, autoNext, sleepAfterEp,
    ontoggleUpscale, ontoggleBoost, ontoggleNight, ontoggleAutoNext, ontoggleSleep,
  ...
  <div class="watch-panel__settings-body">
    ...
    <div class="watch-panel__setting-row watch-panel__setting-row--toggle">
      <div class="watch-panel__setting-label">
        ...
        Буст громкости <span class="watch-panel__setting-hint">до 200%</span>
      </div>
      <div
        role="switch"
        tabindex="0"
        aria-checked={boost}
        class="watch-panel__toggle {boost ? 'watch-panel__toggle--on' : ''}"
        onclick={(e) => { e.stopPropagation(); ontoggleBoost(); }}
        onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); e.preventDefault(); ontoggleBoost(); } }}
      ></div>
    </div>
    ...
    <div class="watch-panel__setting-row watch-panel__setting-row--toggle">
      <div class="watch-panel__setting-label">
        ...
        Ночной режим <span class="watch-panel__setting-hint">ровная громкость</span>
      </div>
      <div
        role="switch"
        tabindex="0"
        aria-checked={nightMode}
        class="watch-panel__toggle {nightMode ? 'watch-panel__toggle--on' : ''}"
        onclick={(e) => { e.stopPropagation(); ontoggleNight(); }}
        onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); e.preventDefault(); ontoggleNight(); } }}
      ></div>
    </div>
    ...
    <div class="watch-panel__setting-row watch-panel__setting-row--toggle">
      <div class="watch-panel__setting-label">
        ...
        Пропускать титры <span class="watch-panel__setting-hint">переход на титрах</span>
      </div>
      <div
        role="switch"
        tabindex="0"
        aria-checked={autoNext}
        class="watch-panel__toggle {autoNext ? 'watch-panel__toggle--on' : ''}"
        onclick={(e) => { e.stopPropagation(); ontoggleAutoNext(); }}
        onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); e.preventDefault(); ontoggleAutoNext(); } }}
      ></div>
    </div>
    ...
    {#if autoNext}
      <div class="watch-panel__setting-row watch-panel__setting-row--toggle">
        <div class="watch-panel__setting-label">
          ...
          Стоп после серии <span class="watch-panel__setting-hint">таймер сна</span>
        </div>
        <div
          role="switch"
          tabindex="0"
          aria-checked={sleepAfterEp}
          class="watch-panel__toggle {sleepAfterEp ? 'watch-panel__toggle--on' : ''}"
          onclick={(e) => { e.stopPropagation(); ontoggleSleep(); }}
          onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.stopPropagation(); e.preventDefault(); ontoggleSleep(); } }}
        ></div>
      </div>
    {/if}
