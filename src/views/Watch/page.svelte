<script lang="ts">
  ...
  import {
    saveWatchProgress, getWatchProgress, clearWatchProgress,
    getEndingLead, learnEndingLead,
  } from '../../utils/watch-progress';
  import { PlayerAudioChain } from './_audio';
  ...
  const audioChain = new PlayerAudioChain();
  ...
  // ── Resume: сохранение позиции просмотра ─────────────────────────────────────
  let lastProgressSaveTs = 0;

  /** Сохранить текущую позицию серии (для последующего resume). */
  function persistWatchProgress(force = false) {
    if (!videoEl || player.useVideo === false) return;
    const now = performance.now();
    if (!force && now - lastProgressSaveTs < 5000) return;
    lastProgressSaveTs = now;
    const t = videoEl.currentTime;
    const d = isFinite(videoEl.duration) ? videoEl.duration : 0;
    if (!Number.isFinite(t)) return;
    saveWatchProgress(watchState.releaseId, watchState.ep, t, d);
  }
  ...
  // ── Автопереход к следующей серии (без досмотра титров) ────────────────────
  let nextCountdownTimer: ReturnType<typeof setInterval> | null = null;
  /** Серия, для которой пользователь закрыл предложение — не навязываемся снова. */
  let nextDismissedForEp = -1;

  function cancelNextCountdown() {
    if (nextCountdownTimer) { clearInterval(nextCountdownTimer); nextCountdownTimer = null; }
    player.nextCountdown = null;
  }

  /** «Отмена» — пользователь хочет досмотреть эндинг этой серии. */
  function dismissNextCountdown() {
    nextDismissedForEp = watchState.ep;
    cancelNextCountdown();
  }

  function startNextCountdown() {
    if (player.nextCountdown !== null) return;
    player.nextCountdown = 10;
    nextCountdownTimer = setInterval(() => {
      if (player.nextCountdown == null) return;
      player.nextCountdown -= 1;
      if (player.nextCountdown <= 0) {
        cancelNextCountdown();
        goToEpisode(watchState.ep + 1);
      }
    }, 1000);
  }

  /**
   * Ручной переход: заодно учимся, где у этого релиза начинаются титры —
   * в следующий раз предложим переход в этот же момент.
   */
  function manualNextEpisode() {
    if (videoEl && isFinite(videoEl.duration) && videoEl.duration > 0
        && videoEl.currentTime > videoEl.duration * 0.6) {
      learnEndingLead(watchState.releaseId, videoEl.duration - videoEl.currentTime);
    }
    cancelNextCountdown();
    goToEpisode(watchState.ep + 1);
  }

  /** Дошли до титров — предлагаем следующую серию. */
  function maybeOfferNextEpisode() {
    if (!player.autoNext || player.sleepAfterEp) return;
    if (!hasNextEp) return;
    if (getCurrentRoomId()) return;            // в лобби переключает ведущий
    if (nextDismissedForEp === watchState.ep) return;
    if (player.nextCountdown !== null) return;
    if (!videoEl || !isFinite(videoEl.duration) || videoEl.duration <= 0) return;
    if (videoEl.currentTime >= videoEl.duration - getEndingLead(watchState.releaseId)) {
      startNextCountdown();
    }
  }
  ...
  function goToEpisode(ep: number) {
    ...
    watchState.ep = ep;
    // Вне лобби — продолжаем серию с сохранённой позиции, если она есть.
    const resumeTime = getCurrentRoomId() ? undefined : getWatchProgress(watchState.releaseId, ep) ?? undefined;
    loadEpisode(parseInt(watchState.releaseId, 10), parseInt(watchState.sourceId, 10), ep, watchState.title, watchState.sourceName, watchState.dubberId, resumeTime);
  ...
  /** Единая точка громкости: 0–100 идёт в элемент, выше — через WebAudio. */
  function setVolume(v: number) {
    const max = player.boost ? 200 : 100;
    const next = Math.max(0, Math.min(max, Math.round(v)));
    player.volume = next;
    if (next > 0 && player.muted) {
      player.muted = false;
      audioChain.setMuted(false);
    }
    audioChain.setVolume(next);
    try { localStorage.setItem(VOLUME_KEY, String(next)); } catch {}
  }

  function toggleMute() {
    player.muted = !player.muted;
    audioChain.setMuted(player.muted);
  }

  function toggleBoost() {
    player.boost = !player.boost;
    if (!player.boost && player.volume > 100) setVolume(100);
    else audioChain.setVolume(player.volume);
    (window as any).electron?.saveSettings?.({ playerBoost: player.boost });
  }

  function toggleNightMode() {
    player.nightMode = !player.nightMode;
    audioChain.setNight(player.nightMode);
    (window as any).electron?.saveSettings?.({ playerNightMode: player.nightMode });
  }

  function toggleAutoNext() {
    player.autoNext = !player.autoNext;
    if (!player.autoNext) cancelNextCountdown();
    (window as any).electron?.saveSettings?.({ playerAutoNext: player.autoNext });
  }

  function toggleSleepAfterEp() {
    player.sleepAfterEp = !player.sleepAfterEp;
    if (player.sleepAfterEp) cancelNextCountdown();
  }
  ...
  function seekToPercent(pct: number) {
    if (!videoEl || isNaN(videoEl.duration)) return;
    videoEl.currentTime = videoEl.duration * Math.max(0, Math.min(1, pct));
    sendToLobby('seek');
    showAndSchedule();
  }
  ...
  function onKeyDown(e: KeyboardEvent) {
    ...
    // Цифры 0–9 — переход на N×10% серии.
    if (/^Digit[0-9]$/.test(e.code)) {
      e.preventDefault();
      seekToPercent(Number(e.code.slice(5)) / 10);
      return;
    }
    ...
    switch (e.code) {
      ...
      case 'KeyF':
        ...
        break;
      case 'KeyN':
        if (hasNextEp) { e.preventDefault(); manualNextEpisode(); }
        break;
      case 'Escape':
        if (player.isFullscreen) { e.preventDefault(); toggleFullscreen(); }
        break;
    }
  }
  ...
  onMount(() => {
    ...
    if ((window as any).electron?.getSettings) {
      (window as any).electron.getSettings().then((s: any) => {
        ...
        player.boost     = s?.playerBoost === true;
        player.nightMode = s?.playerNightMode === true;
        player.autoNext  = s?.playerAutoNext !== false; // по умолчанию включено
        if (player.nightMode) audioChain.setNight(true);
        if (gpuAvailable) {
    ...
    ;(window as any).anixApi.release.getEpisode(
      ...
      await tick();
      // Resume: при обычном открытии (не в лобби) перематываем на сохранённую позицию.
      const resumeTime = getCurrentRoomId() ? undefined : getWatchProgress(watchState.releaseId, watchState.ep) ?? undefined;
      applyVideoAndUI(pUrl, uv, watchState.ep, watchState.title, watchState.sourceName, watchState.dubberId, resumeTime);
      refreshDubberNameFromApi();
      fetchEpisodesSilently();

      if (uv) {
        audioChain.attach(videoEl);
        audioChain.setVolume(player.volume);
        audioChain.setNight(player.nightMode);
        ...
        videoEl.addEventListener('pause', () => { player.paused = true;  sendToLobby('pause'); persistWatchProgress(true); });
        videoEl.addEventListener('ended', () => {
          clearWatchProgress(watchState.releaseId, watchState.ep);
          // Досмотрели до самого конца — уходим сразу, без отсчёта.
          if (player.autoNext && !player.sleepAfterEp && hasNextEp && !getCurrentRoomId()) {
            cancelNextCountdown();
            goToEpisode(watchState.ep + 1);
          }
        });
    ...
    // Точное сохранение позиции при полном закрытии окна.
    const onBeforeUnload = () => persistWatchProgress(true);
    ...
    <div class="watch-page__player-wrap">
      ...
      {:else}
        ...
        <div class="watch-page__gui-overlay" class:watch-page__gui-overlay--hidden={!player.overlayVisible}>
          ...
          <!-- ── Автопереход: предложение следующей серии на титрах ─────── -->
          {#if player.nextCountdown !== null}
            <div class="watch-next" role="status" aria-live="polite">
              <div class="watch-next__label">Следующая серия</div>
              <div class="watch-next__ep">Серия {watchState.ep + 1}</div>
              <div class="watch-next__row">
                <button
                  type="button"
                  class="watch-next__go"
                  onclick={(e) => { e.stopPropagation(); manualNextEpisode(); }}
                >
                  <span class="watch-next__go-ring" style:--p={`${(10 - (player.nextCountdown ?? 0)) * 10}%`}></span>
                  Смотреть · {player.nextCountdown}
                </button>
                <button
                  type="button"
                  class="watch-next__cancel"
                  onclick={(e) => { e.stopPropagation(); dismissNextCountdown(); }}
                >
                  Досмотреть
                </button>
              </div>
          ...
          <div class="watch-page__bottom-controls">
            ...
            <ActionsBar
              ...
              sleepAfterEp={player.sleepAfterEp}
              {hasNextEp}
              ...
              ontoggleSleep={toggleSleepAfterEp}
              onnextEpisode={manualNextEpisode}
