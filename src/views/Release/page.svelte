<script lang="ts">
  ...
  import { setAccentFromImage, clearAccent } from '../../services/dynamicAccent';
  ...
  onMount(async () => {
    ...
    try {
      ...
      // Тонируем весь интерфейс под цвет постера этого аниме.
      if (posterVal) void setAccentFromImage(posterVal);
    } catch (err) {
      errorMsg  = String(err);
      loadState = 'error';
    }
...
<div class="view view-release">
  ...
  {:else if release}
    <section class="release-page">
      ...
      <!-- Head: poster + info -->
      <ReleaseHead
        {posterUrl} backdropUrl={screenshots[0] ?? ''} {title} {titleRu} {titleOriginal} {titleAlt}
