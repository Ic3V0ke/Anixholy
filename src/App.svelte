<script lang="ts">
  ...
  // Single-window: «плеер открыт» = находимся на маршруте /watch главного окна.
  // Стор питает лобби-синхронизацию (sendPlayerViewActive) и виджет «Сейчас смотрят».
  $effect(() => {
    isPlayerWindowOpen.set(isWatchRoute);
    // Discord: на /watch показываем активность просмотра, иначе — навигацию по меню.
    window.electron?.setDiscordActivityContext?.(isWatchRoute ? 'player' : 'main');
    if (!isWatchRoute) {
      // Уходя из плеера, снимаем полноэкранный режим окна, если он был включён.
      window.electron?.setMainFullScreen?.(false);
    }
  });
