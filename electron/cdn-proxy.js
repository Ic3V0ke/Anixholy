...
// Неудачные загрузки помним недолго, чтобы не устраивать шторм ретраев.
const NEGATIVE_TTL_MS = 60 * 1000;
/** @type {Map<string, number>} */
const negativeCache = new Map();

// Жёсткие лимиты: main-процесс обслуживает ввод окна, его нельзя перегружать.
...
async function fetchCdnAsset(url) {
  ...
  if (failedAt && Date.now() - failedAt < NEGATIVE_TTL_MS) {
    throw new Error(`CDN recently failed for ${url}`);
  }
  ...
  try {
    ...
    if (negativeCache.size > 512) {
      const first = negativeCache.keys().next().value;
      if (first != null) negativeCache.delete(first);
    }
    throw err;
  } finally {
    releaseFetchSlot();
