...
/**
 * Куда слать события «для плеера».
 * Single-window: плеер живёт на маршруте /watch главного окна, поэтому при
 * отсутствии отдельного окна плеера события уходят в главное окно — preload
 * поднимет их как CustomEvent, и страница Watch обработает их как раньше.
 */
function playerTarget() {
  if (state.playerWindowRef && !state.playerWindowRef.isDestroyed()) return state.playerWindowRef;
...
// Legacy-канал: раньше открывал отдельное окно плеера. Оставлен для
// совместимости — теперь просто переключает контент плеера в главном окне.
ipcMain.handle('player:openWindow', async (_, params) => {
  ...
  target.webContents.send('player:changeContent', { ...safe, local: true });
  target.focus();
});
...
ipcMain.on('player:syncState', async (_, playback) => {
  ...
  if (sameContent) {
    ...
    target.webContents.send('player:applySync', params);
  } else {
    // Different content — change dynamically without closing/reopening
    state.currentPlayerPlayback = incomingContent;
    target.webContents.send('player:changeContent', params);
  }
});

// ── Upscale settings sync: Main window → Player view ──
ipcMain.on('upscale:applySettings', (_, settings) => {
  const target = playerTarget();
  if (target) target.webContents.send('upscale:settingsChanged', settings);
});

// ── Lobby proposal IPC forwarding ──
// Main window → Player view (proposal events)
ipcMain.on('lobby:proposalToPlayer', (_, data) => {
  const target = playerTarget();
  if (target) target.webContents.send('lobby:proposal', data);
});

// Main window → Player view (activity feed & participant list)
ipcMain.on('lobby:activityToPlayer', (_, data) => {
  const target = playerTarget();
  if (target) target.webContents.send('lobby:activityFeed', data);
});

ipcMain.on('lobby:participantsToPlayer', (_, participants) => {
  const target = playerTarget();
  if (target) target.webContents.send('lobby:participantsList', participants);
});
...
ipcMain.on('lobby:waitingOverlayToPlayer', (_, payload) => {
  const target = playerTarget();
  if (target) target.webContents.send('lobby:playerWaitingOverlay', payload);
});
...
ipcMain.on('player:close', (event) => {
  ...
  // Закрываем только отдельное окно плеера; главное окно (single-window
  // плеер на /watch) по этому каналу закрывать нельзя.
  if (win && win === state.playerWindowRef && !win.isDestroyed()) win.close();
});
...
// Single-window: плеер живёт на /watch главного окна, отдельного окна нет.
// Renderer сообщает, какой «контекст» активен, чтобы Discord показывал либо
// активность просмотра (setWatching), либо активность навигации по меню.
ipcMain.on('discord:focusContext', (_, ctx) => {
  if (!discordRpc) return;
  const context = ctx === 'player' ? 'player' : 'main';
  discordRpc.focusWindow(context);
  if (context === 'main' && discordRpc.clearPlayerActivity) {
    // Уходя из плеера, сбрасываем «залипшую» активность просмотра.
    discordRpc.clearPlayerActivity();
  }
});
