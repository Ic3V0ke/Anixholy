...
// statSync на каждую запись блокирует event loop main-процесса —
// проверяем ротацию не чаще, чем раз в N записей на файл.
const ROTATE_CHECK_EVERY = 64;
const _rotateCounters = new Map();

function rotateIfNeeded(filePath) {
  const n = (_rotateCounters.get(filePath) ?? 0) + 1;
  _rotateCounters.set(filePath, n);
  if (n % ROTATE_CHECK_EVERY !== 1) return;
  try {
...
// Rate-limit одинаковых сообщений: шторм ошибок (например, недоступный CDN)
// не должен превращаться в шторм синхронных записей на диск.
const _errBurst = new Map();
function errorBurstExceeded(key) {
  const now = Date.now();
  const rec = _errBurst.get(key);
  if (!rec || now - rec.windowStart > 5000) {
    _errBurst.set(key, { windowStart: now, count: 1 });
    if (_errBurst.size > 200) _errBurst.clear();
    return false;
  }
  rec.count += 1;
  return rec.count > 20;
...
function writeLog(file, level, ch, msg, data, sync = false) {
  ...
  try {
    if (level === 'ERROR' && errorBurstExceeded(`${ch}|${msg}`)) return;
