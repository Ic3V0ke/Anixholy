...
function createMainWindow(deps) {
  ...
  // Плеер живёт на /watch главного окна — сообщаем странице о fullscreen.
  state.mainWindow.on('enter-full-screen', () => {
    state.mainWindow?.webContents.send('player:fullscreen', true);
  });
  state.mainWindow.on('leave-full-screen', () => {
    state.mainWindow?.webContents.send('player:fullscreen', false);
  });
